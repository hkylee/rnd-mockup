...
import { PromoBanner } from "@/components/library/ogn/MYBEN/promo-banner";
import { CardPoints } from "@/components/library/ogn/MYBEN/card-points";
import { CardBarcode } from "@/components/library/ogn/MYBEN/card-barcode";
import { CardBrandList } from "@/components/library/ogn/MYBEN/card-brand-list";
import { CardPromo } from "@/components/library/ogn/MYBEN/card-promo";
import { CardMovieList } from "@/components/library/ogn/MYBEN/card-movie-list";
import { CardCouponList } from "@/components/library/ogn/MYBEN/card-coupon-list";
import { TabBar } from "@/components/library/ogn/tab-bar";

const SLUG = "MYBEN/main";
  ...
  return (
    ...
    <div style={embed ? { minHeight: "100vh", background: "#fff", display: "flex", justifyContent: "center", alignItems: "flex-start", padding: 0 } : { minHeight: "100vh", background: "#1f2937", display: "flex", justifyContent: "center", alignItems: "flex-start", padding: 32 }}>
      <div style={embed ? { width: 375, height: 812, background: "var(--semantic-color-background-page-home, #ffffff)", overflow: "hidden", display: "flex", flexDirection: "column" } : { width: 375, height: 812, background: "var(--semantic-color-background-page-home, #ffffff)", borderRadius: 24, overflow: "hidden", boxShadow: "0 8px 24px rgba(0,0,0,0.2)", display: "flex", flexDirection: "column" }}>
        <div data-component="page/MYBEN/main" style={{ ...{ display: "flex", flexDirection: "column", justifyContent: "flex-start", alignItems: "flex-start", gap: "var(--foundation-dimension-spacing-none)", paddingTop: "var(--foundation-dimension-spacing-none)", paddingRight: "var(--foundation-dimension-spacing-none)", paddingBottom: "var(--foundation-dimension-spacing-none)", paddingLeft: "var(--foundation-dimension-spacing-none)", width: "100%", height: "var(--foundation-dimension-size-screen-mobile-height)", background: "var(--semantic-color-background-page-home)", minHeight: "100%" }, height: "100%", overflowY: "auto", overflowX: "hidden" }} onClick={(e) => {
          ...
          <Gnb data-id="gnb" />
          <div data-id="content" style={{ display: "flex", flexDirection: "column", justifyContent: "flex-start", alignItems: "center", gap: "var(--semantic-layout-card-gap)", paddingTop: "var(--foundation-dimension-spacing-none)", paddingRight: "var(--foundation-dimension-spacing-md)", paddingBottom: "var(--foundation-dimension-spacing-xs)", paddingLeft: "var(--foundation-dimension-spacing-md)", width: "100%", height: "auto", flex: "1 1 0%", minWidth: 0 }}>
            <PromoBanner data-id="promo-banner" />
            <CardPoints data-id="card-points" />
            <CardBarcode data-id="card-barcode" />
            <CardBrandList data-id="card-brand-list" />
            <CardPromo data-id="card-promo" />
            <CardMovieList data-id="card-movie-list" />
            <CardCouponList data-id="card-coupon-list" />
          </div>
          <TabBar data-id="tab-bar" />
        </div>
      </div>
    </div>
