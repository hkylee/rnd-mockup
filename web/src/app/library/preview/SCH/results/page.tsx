...
import { SearchBar } from "@/components/library/ogn/SCH/search-bar";
import { Tabs } from "@/components/library/ogn/SCH/tabs";
import { AnswerCard } from "@/components/library/ogn/SCH/answer-card";
import { QuickActionCard } from "@/components/library/ogn/SCH/quick-action-card";
import { ResultList } from "@/components/library/ogn/SCH/result-list";
import { CompareTray } from "@/components/library/ogn/SCH/compare-tray";
import { FooterCs } from "@/components/library/ogn/footer-cs";

const SLUG = "SCH/results";
  ...
  return (
    ...
    <div style={embed ? { minHeight: "100vh", background: "#fff", display: "flex", justifyContent: "center", alignItems: "flex-start", padding: 0 } : { minHeight: "100vh", background: "#1f2937", display: "flex", justifyContent: "center", alignItems: "flex-start", padding: 32 }}>
      <div style={embed ? { width: 375, height: 812, background: "var(--semantic-color-background-page-home, #ffffff)", overflow: "hidden", display: "flex", flexDirection: "column" } : { width: 375, height: 812, background: "var(--semantic-color-background-page-home, #ffffff)", borderRadius: 24, overflow: "hidden", boxShadow: "0 8px 24px rgba(0,0,0,0.2)", display: "flex", flexDirection: "column" }}>
        <div data-component="page/SCH/results" style={{ ...{ display: "flex", flexDirection: "column", justifyContent: "flex-start", alignItems: "flex-start", gap: "var(--foundation-dimension-spacing-none)", paddingTop: "var(--foundation-dimension-spacing-none)", paddingRight: "var(--foundation-dimension-spacing-none)", paddingBottom: "var(--foundation-dimension-spacing-none)", paddingLeft: "var(--foundation-dimension-spacing-none)", width: "100%", height: "var(--foundation-dimension-size-screen-mobile-height)", background: "var(--semantic-color-background-page-sub)", minHeight: "100%" }, height: "100%", overflowY: "auto", overflowX: "hidden" }} onClick={(e) => {
          ...
          <SearchBar data-id="search-bar" {...{"placeholder": "5G 요금제"}} style={{ alignSelf: "stretch", position: "sticky", bottom: 0, zIndex: 10 }} />
          <Tabs data-id="tabs" {...{"active": "0"}} style={{ alignSelf: "stretch" }} />
          <div data-id="content" style={{ display: "flex", flexDirection: "column", justifyContent: "flex-start", alignItems: "flex-start", gap: "var(--foundation-dimension-spacing-lg)", paddingTop: "var(--semantic-layout-screen-padding-default)", paddingRight: "var(--semantic-layout-screen-padding-default)", paddingBottom: "var(--semantic-layout-screen-padding-default)", paddingLeft: "var(--semantic-layout-screen-padding-default)", width: "100%", height: "auto", alignSelf: "stretch", flex: "1 1 0%", minWidth: 0 }}>
            <AnswerCard data-id="answer-card" {...{"headline": "이번 달 요금", "summary": "73,500원 (납부 예정 7월 25일)"}} style={{ alignSelf: "stretch" }} />
            <QuickActionCard data-id="quick-action" {...{"headline": "데이터 리필 받기", "description": "이번 달 데이터 리필권 1회 사용 가능"}} style={{ alignSelf: "stretch" }} />
            <ResultList data-id="result-list-exact" style={{ alignSelf: "stretch" }} />
            <ResultList data-id="result-list-related" {...{"label": "연관 결과"}} style={{ alignSelf: "stretch" }} />
          </div>
          <CompareTray data-id="compare-tray" {...{"count": "2개 선택됨"}} style={{ alignSelf: "stretch", position: "sticky", bottom: 0, zIndex: 10 }} />
