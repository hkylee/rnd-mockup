...
function usage() {
  ...
  console.error("  node scripter/bundle.js --all     [--dry]      # batch: 전체 (atom+icon+mol+ogn+page)");
  console.error("  node scripter/bundle.js --library [--dry]      # batch: atom + icon + mol + ogn");
  console.error("  node scripter/bundle.js --pages   [--dry]      # batch: 모든 page");
  console.error("  node scripter/bundle.js --group <atom|mol|ogn|sb|page> [--dry]  # batch: 단일 category");
...
function buildSpecRegistry() {
  const cats = ["atom", "mol", "ogn", "sb", "page"];
  const reg = {};
  for (const c of cats) {
...
function bundleLibrary(dry) {
  ...
  const specs = [...collectSpecs("atom"), ...collectSpecs("mol"), ...collectSpecs("ogn")];
  const icons = loadIcons();
  const banner = [
    "// =============================================",
    "// AUTO-GENERATED — BATCH MODE (--library)",
    "// atom + icon + mol + ogn 통합 — Run 1회로 전체 라이브러리 빌드",
...
function bundleGroup(category, dry) {
  if (!["atom", "mol", "ogn", "sb", "page"].includes(category)) {
    console.error("Unknown category:", category, "— atom/mol/ogn/sb/page 중 하나여야 합니다");
    process.exit(2);
  }
  const ds = readDsCombined();
  const specs = collectSpecs(category);
  const icons = (category === "atom") ? loadIcons() : [];
  const banner = [
    "// =============================================",
...
function bundleChanged(dry) {
  ...
  if (touchIconRegistry && touchedSpecs.length === 0) {
    console.log("[--changed] icon-registry 만 변경 → --group atom");
    return bundleGroup("atom", dry);
  }
  ...
  } else if (specs.some(s => (s.name || "").startsWith("atom/"))) {
    icons = loadIcons();
  }
...
function bundleComponent(specArg, dry) {
  ...
  const banner = [
    "// =============================================",
    "// AUTO-GENERATED — DO NOT EDIT",
    "// source spec:  component-specs/" + specArg + ".json",
