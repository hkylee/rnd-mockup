---
유즈케이스_ID: US-MBR-CS-001
액터: 고객
설명: 비회원 고객이 회원으로 전환되기 위해 수행하는 상위 유즈케이스
프로세스_정의_대상: Y
---

# Usecase: US-MBR-CS-001 — 회원 가입
...
graph LR
    ...
    PR_MBR_CS_001_04 --> PR_MBR_CS_001_05
    End((종료))
    PR_MBR_CS_001_05 --> End
```

## Process: PR-MBR-CS-001-01 — 약관 동의 {#process-PR-MBR-CS-001-01}

```yaml
프로세스_ID: PR-MBR-CS-001-01
프로세스명: 약관 동의
설명: 회원 가입에 필요한 약관을 확인하고 동의한다.
관련_기능: [FN-MBR-COM-003, FN-MBR-COM-004, FN-MBR-COM-005]
```
...
## Process: PR-MBR-CS-001-02 — 개인정보 입력 {#process-PR-MBR-CS-001-02}

```yaml
프로세스_ID: PR-MBR-CS-001-02
프로세스명: 개인정보 입력
설명: 가입에 필요한 기본 정보를 입력한다.
관련_기능: [FN-MBR-JOIN-002, FN-MBR-JOIN-001]
```
...
### Function: FN-MBR-JOIN-002

```yaml
기능_ID: FN-MBR-JOIN-002
기능명: 회원정보 입력 및 검증
설명: 가입에 필요한 기본 회원정보를 입력받고 형식·중복을 검증한다.
관련_정책_그룹: [PG-MBR-INFO-001, PG-MBR-INFO-002, PG-MBR-INFO-003]
```

| 항목 | 내용 |
| --- | --- |
| 입력 정보 | 이름, 아이디, 비밀번호, 이메일, 연락처, 마케팅 수신값, 세션ID |
| 세부 기능 구성 | 필수 입력값 확인 입력 형식 검증 중복 정보 확인 가입 세션 임시 저장 |
| 출력 정보 | 입력검증결과, 중복여부, 임시저장ID, 다음 이동 경로 |
| 처리 흐름 | (상태) 필수 정보 입력 → (액션) 형식·필수값 검증 → (결과) 입력 가능 상태 반환 (상태) 중복 대상 정보 존재 → (액션) 아이디·연락처·이메일 중복 확인 → (결과) 수정 요청 (상태) 검증 통과 → (액션) 가입 세션에 저장 → (결과) 다음 단계 허용 |
| 실패/예외 케이스 | 필수값 누락: 누락 항목 표시 형식 오류: 항목별 오류 안내 중복값 존재: 대체값 입력 요청 |

#### Policy Group: PG-MBR-INFO-001

```yaml
정책_ID: PG-MBR-INFO-001
정책명: 회원 정보 입력 항목 정책
설명: 회원 가입 또는 재가입 과정에서 입력받아야 하는 회원 정보 항목과 필수·선택 여부를 정의하는 정책 그룹이다.
```

| Policy Item ID | 정책 항목명 | 정책 항목 |
| --- | --- | --- |
| `POL-MBR-INFO-001-01` | 회원 가입 필수 입력 항목 | - 아이디, 비밀번호, 이메일, 연락처 |
| `POL-MBR-INFO-001-02` | 회원 가입 선택 입력 항목 | - 닉네임, 관심 혜택, 추천 수신 선호 항목 |
| `POL-MBR-INFO-001-03` | 본인인증 결과 연계 항목 | - 이름, 생년월일, 성별, 내외국인 구분, 휴대폰번호, CI, DI |
| `POL-MBR-INFO-001-04` | 고객 유형 구분 항목 | - 일반 개인, 미성년자, 외국인 |
| `POL-MBR-INFO-001-05` | 미성년자 추가 입력 항목 | - 법정대리인 이름, 법정대리인 휴대폰번호, 고객과의 관계 |
| `POL-MBR-INFO-001-06` | 외국인 추가 입력 항목 | - 국적, 외국인등록번호 대체 식별값, 체류자격 확인 결과 |
| `POL-MBR-INFO-001-07` | 고객 직접 입력 제외 항목 | - CI, DI |
| `POL-MBR-INFO-001-08` | 항목 노출 기준 | - 고객 유형, 본인인증 결과, 가입 경로 |
| `POL-MBR-INFO-001-09` | 입력 생략 가능 항목 | - 본인인증 결과 연계 항목, 임시저장 복원 항목 |
| `POL-MBR-INFO-001-10` | 입력 임시저장 대상 항목 | - 아이디, 이메일, 연락처 |
| `POL-MBR-INFO-001-11` | 입력 임시저장 제외 항목 | - 비밀번호, CI, DI |
| `POL-MBR-INFO-001-12` | 가입 완료 저장 항목 | - 아이디, 이메일, 연락처, 이름, 생년월일, 성별, 내외국인 구분, 휴대폰번호, CI, DI |

#### Policy Group: PG-MBR-INFO-002

```yaml
정책_ID: PG-MBR-INFO-002
정책명: 회원 정보 입력값 검증 정책
설명: 회원 정보 입력값의 형식, 길이, 필수값, 정합성 검증 기준을 정의하는 정책 그룹이다.
```

| Policy Item ID | 정책 항목명 | 정책 항목 |
| --- | --- | --- |
| `POL-MBR-INFO-002-01` | 필수값 검증 대상 | - 아이디, 비밀번호, 이메일, 연락처 |
| `POL-MBR-INFO-002-02` | 필수값 누락 처리 | - 다음 단계 진행 불가 |
| `POL-MBR-INFO-002-03` | 아이디 허용 문자 | - 영문 대문자, 영문 소문자, 숫자 |
| `POL-MBR-INFO-002-04` | 아이디 길이 | - 6~20자 |
| `POL-MBR-INFO-002-05` | 비밀번호 길이 | - 10~20자 |
| `POL-MBR-INFO-002-06` | 비밀번호 문자 조합 | - 영문 대문자, 영문 소문자, 숫자, 특수문자 중 3종 이상 조합 |
| `POL-MBR-INFO-002-07` | 이메일 형식 | - 이메일 주소 형식 |
| `POL-MBR-INFO-002-08` | 연락처 형식 | - 휴대폰번호 숫자 |
| `POL-MBR-INFO-002-09` | 공백 입력 허용 여부 | - 불가 |
| `POL-MBR-INFO-002-10` | 본인인증 연계 항목 수정 허용 여부 | - 불가 |
| `POL-MBR-INFO-002-11` | 검증 실패 처리 | - 다음 단계 진행 불가 |
| `POL-MBR-INFO-002-12` | 검증 시점 | - 입력 중, 다음 단계 이동 시, 가입 완료 요청 시 |
| `POL-MBR-INFO-002-13` | 오류 안내 문구 | - 입력값을 다시 확인해 주세요. |

#### Policy Group: PG-MBR-INFO-003

```yaml
정책_ID: PG-MBR-INFO-003
정책명: 회원 식별정보 중복 확인 정책
설명: CI, DI, 휴대폰번호 등 식별정보의 중복 확인 기준을 정의하는 정책 그룹이다.
```

| Policy Item ID | 정책 항목명 | 정책 항목 |
| --- | --- | --- |
| `POL-MBR-INFO-003-01` | 중복 확인 대상 식별정보 | - CI, DI, 아이디, 휴대폰번호, 이메일 |
| `POL-MBR-INFO-003-02` | CI 중복 확인 기준 | - 동일 CI 기준 1인 1계정 |
| `POL-MBR-INFO-003-03` | DI 중복 확인 기준 | - 동일 DI 중복 가입 불가 |
| `POL-MBR-INFO-003-04` | 아이디 중복 처리 | - 사용 불가 |
| `POL-MBR-INFO-003-05` | 휴대폰번호 중복 처리 | - 기존 회원 확인 |
| `POL-MBR-INFO-003-06` | 이메일 중복 처리 | - 동일 이메일 기존 회원 존재 시 기존 계정 확인 안내 |
| `POL-MBR-INFO-003-07` | 기존 정상 회원 식별 시 처리 | - 로그인, 아이디 찾기 |
| `POL-MBR-INFO-003-08` | 기존 휴면 회원 식별 시 처리 | - 휴면 해제 |
| `POL-MBR-INFO-003-09` | 기존 탈퇴 회원 식별 시 처리 | - 재가입 가능 여부 확인 |
| `POL-MBR-INFO-003-10` | 중복 확인 수행 시스템 | - BSS |
| `POL-MBR-INFO-003-11` | 중복 확인 시점 | - 회원 정보 입력 완료 후, 가입 처리 전 |
| `POL-MBR-INFO-003-12` | 중복 확인 실패 시 처리 | - 다음 단계 진행 불가 |

### Function: FN-MBR-JOIN-001

```yaml
기능_ID: FN-MBR-JOIN-001
기능명: 가입 진입 조건 확인
...
| 세부 기능 구성 | 진입 채널 확인 로그인 상태 확인 기존 회원 여부 확인 가입 가능 경로 분기 |
| 출력 정보 | 진입가능여부, 진입제한사유, 시작 프로세스ID, 다음 화면 |
| 처리 흐름 | (상태) 미로그인 고객 진입 → (액션) 가입 가능 업무인지 확인 → (결과) 가입 플로우 시작 (상태) 기존 로그인 회원 진입 → (액션) 현재 회원 상태 확인 → (결과) 가입 대신 적정 업무 안내 (상태) 제한 대상 진입 → (액션) 제한 사유 표시 → (결과) 진행 제한 |
| 실패/예외 케이스 | 이미 가입된 회원: 로그인 또는 내정보 안내 휴면 회원: 휴면 해제 안내 재가입 제한 대상: 제한 안내 |
...
## Process: PR-MBR-CS-001-03 — 본인인증 {#process-PR-MBR-CS-001-03}

```yaml
프로세스_ID: PR-MBR-CS-001-03
프로세스명: 본인인증
설명: 가입자의 본인 여부를 확인한다.
...
## Process: PR-MBR-CS-001-04 — 회원 검증 {#process-PR-MBR-CS-001-04}

```yaml
프로세스_ID: PR-MBR-CS-001-04
프로세스명: 회원 검증
설명: 기존 가입 여부와 가입 가능 조건을 확인한다.
관련_기능: [FN-MBR-COM-001, FN-MBR-COM-004, FN-MBR-JOIN-003]
```
...
### Function: FN-MBR-JOIN-003

```yaml
기능_ID: FN-MBR-JOIN-003
기능명: 가입 가능 여부 판정
설명: 본인인증 결과와 기존 이력을 기준으로 신규 가입 가능 여부를 판정한다.
관련_정책_그룹: [PG-MBR-STAT-001, PG-MBR-JOIN-001, PG-MBR-ROUTE-001]
```

| 항목 | 내용 |
| --- | --- |
| 입력 정보 | CI/DI, 인증결과, 고객유형, 기존회원상태, 탈퇴이력 |
| 세부 기능 구성 | 기존 회원 상태 확인 탈퇴 이력 확인 재가입 제한 확인 가입 가능 여부 반환 |
| 출력 정보 | 가입가능여부, 판정사유, 전환 업무, 제한 종료일, 안내 메시지 |
| 처리 흐름 | (상태) 동일 CI 기존 회원 없음 → (액션) 신규 가입 가능 판정 → (결과) 가입 처리 단계 허용 (상태) 휴면 회원 존재 → (액션) 가입 대신 휴면 해제 경로 제시 → (결과) 휴면 해제 유도 (상태) 재가입 제한 존재 → (액션) 제한 사유 확인 → (결과) 가입 제한 안내 |
| 실패/예외 케이스 | 기존 정상 회원: 가입 진행 불가 탈퇴유예 대상: 철회 또는 재가입제한 안내 판정 시스템 오류: 재시도 안내 |
...
## Process: PR-MBR-CS-001-05 — 가입 처리 {#process-PR-MBR-CS-001-05}

```yaml
프로세스_ID: PR-MBR-CS-001-05
프로세스명: 가입 처리
설명: 회원 생성 및 후속 처리를 수행한다.
관련_기능: [FN-MBR-JOIN-004, FN-MBR-JOIN-005, FN-MBR-COM-005, FN-MBR-COM-006]
```
...
### Function: FN-MBR-JOIN-004

```yaml
기능_ID: FN-MBR-JOIN-004
기능명: 회원 계정 생성
설명: 검증이 완료된 고객의 통합 회원 계정을 생성한다.
관련_정책_그룹: [PG-MBR-ACCT-001, PG-MBR-PROF-001, PG-MBR-SESS-001]
```

| 항목 | 내용 |
| --- | --- |
| 입력 정보 | 회원정보, CI/DI, 약관동의이력ID, 인증세션ID, 채널 |
| 세부 기능 구성 | 회원ID 발급 CI/DI 매핑 기본 프로필 생성 정상 회원 상태 생성 |
| 출력 정보 | 회원ID, 회원상태코드, 계정생성시각, 기본프로필ID, 세션전환여부 |
| 처리 흐름 | (상태) 모든 선행 조건 충족 → (액션) 통합 회원 계정 생성 → (결과) 회원ID 발급 (상태) CI/DI 매핑 필요 → (액션) 식별정보와 회원ID 연결 → (결과) 정상 회원 상태 생성 (상태) 프로필 기본값 필요 → (액션) 기본 설정 생성 → (결과) 초기 프로필 저장 |
| 실패/예외 케이스 | 계정 생성 실패: 가입 완료 처리 중단 식별정보 중복: 회원 상태 재조회 동의 이력 누락: 약관 단계 재진입 |
...
### Function: FN-MBR-JOIN-005

```yaml
기능_ID: FN-MBR-JOIN-005
기능명: 가입 완료 처리
...
| 세부 기능 구성 | 가입 결과 확정 가입 완료 화면 제공 가입 완료 알림 발송 로그인 세션 전환 |
| 출력 정보 | 가입완료여부, 로그인세션ID, 완료안내문구, 후속 액션, 알림결과 |
| 처리 흐름 | (상태) 가입 성공 → (액션) 가입 완료 화면과 알림 발송 → (결과) 정상 회원 이용 가능 (상태) 자동 로그인 허용 → (액션) 로그인 세션 생성 → (결과) 홈 또는 목적지 이동 (상태) 후속 안내 대상 → (액션) 내정보 점검·혜택 안내 제공 → (결과) 후속 액션 노출 |
| 실패/예외 케이스 | 가입 완료 알림 실패: 화면 안내 유지 세션 생성 실패: 로그인 재시도 안내 중복 완료 요청: 기존 가입 결과 반환 |
...
- 정의된 ID: `FN-MBR-COM-001`, `FN-MBR-COM-002`, `FN-MBR-COM-003`, `FN-MBR-COM-004`, `FN-MBR-COM-005`, `FN-MBR-COM-006`, `FN-MBR-JOIN-001`, `FN-MBR-JOIN-002`, `FN-MBR-JOIN-003`, `FN-MBR-JOIN-004`, `FN-MBR-JOIN-005`, `PG-MBR-ACCT-001`, `PG-MBR-AUTH-001`, `PG-MBR-AUTH-002`, `PG-MBR-AUTH-005`, `PG-MBR-AUTH-006`, `PG-MBR-AUTH-007`, `PG-MBR-DORM-001`, `PG-MBR-DORM-002`, `PG-MBR-DORM-003`, `PG-MBR-INFO-001`, `PG-MBR-INFO-002`, `PG-MBR-INFO-003`, `PG-MBR-JOIN-001`, `PG-MBR-LOGIN-001`, `PG-MBR-PROF-001`, `PG-MBR-REJOIN-001`, `PG-MBR-REJOIN-004`, `PG-MBR-REJOIN-005`, `PG-MBR-ROUTE-001`, `PG-MBR-SESS-001`, `PG-MBR-SESS-002`, `PG-MBR-STAT-001`, `PG-MBR-TERM-001`, `PG-MBR-TERM-002`, `PG-MBR-TERM-003`, `POL-MBR-ACCT-001-01`, `POL-MBR-ACCT-001-02`, `POL-MBR-ACCT-001-03`, `POL-MBR-ACCT-001-04`, `POL-MBR-ACCT-001-05`, `POL-MBR-ACCT-001-06`, `POL-MBR-ACCT-001-07`, `POL-MBR-ACCT-001-08`, `POL-MBR-ACCT-001-09`, `POL-MBR-ACCT-001-10`, `POL-MBR-ACCT-001-11`, `POL-MBR-ACCT-001-12`, `POL-MBR-AUTH-001-01`, `POL-MBR-AUTH-001-02`, `POL-MBR-AUTH-001-03`, `POL-MBR-AUTH-001-04`, `POL-MBR-AUTH-001-05`, `POL-MBR-AUTH-001-06`, `POL-MBR-AUTH-001-07`, `POL-MBR-AUTH-001-08`, `POL-MBR-AUTH-002-01`, `POL-MBR-AUTH-002-02`, `POL-MBR-AUTH-002-03`, `POL-MBR-AUTH-002-04`, `POL-MBR-AUTH-002-05`, `POL-MBR-AUTH-002-06`, `POL-MBR-AUTH-002-07`, `POL-MBR-AUTH-002-08`, `POL-MBR-AUTH-002-09`, `POL-MBR-AUTH-005-01`, `POL-MBR-AUTH-005-02`, `POL-MBR-AUTH-005-03`, `POL-MBR-AUTH-005-04`, `POL-MBR-AUTH-005-05`, `POL-MBR-AUTH-005-06`, `POL-MBR-AUTH-005-07`, `POL-MBR-AUTH-006-01`, `POL-MBR-AUTH-006-02`, `POL-MBR-AUTH-006-03`, `POL-MBR-AUTH-006-04`, `POL-MBR-AUTH-006-05`, `POL-MBR-AUTH-006-06`, `POL-MBR-AUTH-006-07`, `POL-MBR-AUTH-007-01`, `POL-MBR-AUTH-007-02`, `POL-MBR-AUTH-007-03`, `POL-MBR-AUTH-007-04`, `POL-MBR-AUTH-007-05`, `POL-MBR-AUTH-007-06`, `POL-MBR-AUTH-007-07`, `POL-MBR-DORM-001-01`, `POL-MBR-DORM-001-02`, `POL-MBR-DORM-001-03`, `POL-MBR-DORM-001-04`, `POL-MBR-DORM-001-05`, `POL-MBR-DORM-001-06`, `POL-MBR-DORM-001-07`, `POL-MBR-DORM-001-08`, `POL-MBR-DORM-001-09`, `POL-MBR-DORM-001-10`, `POL-MBR-DORM-001-11`, `POL-MBR-DORM-001-12`, `POL-MBR-DORM-002-01`, `POL-MBR-DORM-002-02`, `POL-MBR-DORM-002-03`, `POL-MBR-DORM-002-04`, `POL-MBR-DORM-002-05`, `POL-MBR-DORM-002-06`, `POL-MBR-DORM-002-07`, `POL-MBR-DORM-002-08`, `POL-MBR-DORM-002-09`, `POL-MBR-DORM-002-10`, `POL-MBR-DORM-002-11`, `POL-MBR-DORM-002-12`, `POL-MBR-DORM-003-01`, `POL-MBR-DORM-003-02`, `POL-MBR-DORM-003-03`, `POL-MBR-DORM-003-04`, `POL-MBR-DORM-003-05`, `POL-MBR-DORM-003-06`, `POL-MBR-DORM-003-07`, `POL-MBR-DORM-003-08`, `POL-MBR-DORM-003-09`, `POL-MBR-DORM-003-10`, `POL-MBR-DORM-003-11`, `POL-MBR-DORM-003-12`, `POL-MBR-INFO-001-01`, `POL-MBR-INFO-001-02`, `POL-MBR-INFO-001-03`, `POL-MBR-INFO-001-04`, `POL-MBR-INFO-001-05`, `POL-MBR-INFO-001-06`, `POL-MBR-INFO-001-07`, `POL-MBR-INFO-001-08`, `POL-MBR-INFO-001-09`, `POL-MBR-INFO-001-10`, `POL-MBR-INFO-001-11`, `POL-MBR-INFO-001-12`, `POL-MBR-INFO-002-01`, `POL-MBR-INFO-002-02`, `POL-MBR-INFO-002-03`, `POL-MBR-INFO-002-04`, `POL-MBR-INFO-002-05`, `POL-MBR-INFO-002-06`, `POL-MBR-INFO-002-07`, `POL-MBR-INFO-002-08`, `POL-MBR-INFO-002-09`, `POL-MBR-INFO-002-10`, `POL-MBR-INFO-002-11`, `POL-MBR-INFO-002-12`, `POL-MBR-INFO-002-13`, `POL-MBR-INFO-003-01`, `POL-MBR-INFO-003-02`, `POL-MBR-INFO-003-03`, `POL-MBR-INFO-003-04`, `POL-MBR-INFO-003-05`, `POL-MBR-INFO-003-06`, `POL-MBR-INFO-003-07`, `POL-MBR-INFO-003-08`, `POL-MBR-INFO-003-09`, `POL-MBR-INFO-003-10`, `POL-MBR-INFO-003-11`, `POL-MBR-INFO-003-12`, `POL-MBR-JOIN-001-01`, `POL-MBR-JOIN-001-02`, `POL-MBR-JOIN-001-03`, `POL-MBR-JOIN-001-04`, `POL-MBR-JOIN-001-05`, `POL-MBR-JOIN-001-06`, `POL-MBR-JOIN-001-07`, `POL-MBR-JOIN-001-08`, `POL-MBR-JOIN-001-09`, `POL-MBR-JOIN-001-10`, `POL-MBR-JOIN-001-11`, `POL-MBR-JOIN-001-12`, `POL-MBR-LOGIN-001-01`, `POL-MBR-LOGIN-001-02`, `POL-MBR-LOGIN-001-03`, `POL-MBR-LOGIN-001-04`, `POL-MBR-LOGIN-001-05`, `POL-MBR-LOGIN-001-06`, `POL-MBR-LOGIN-001-07`, `POL-MBR-LOGIN-001-08`, `POL-MBR-LOGIN-001-09`, `POL-MBR-LOGIN-001-10`, `POL-MBR-LOGIN-001-11`, `POL-MBR-LOGIN-001-12`, `POL-MBR-PROF-001-01`, `POL-MBR-PROF-001-02`, `POL-MBR-PROF-001-03`, `POL-MBR-PROF-001-04`, `POL-MBR-PROF-001-05`, `POL-MBR-PROF-001-06`, `POL-MBR-PROF-001-07`, `POL-MBR-PROF-001-08`, `POL-MBR-PROF-001-09`, `POL-MBR-PROF-001-10`, `POL-MBR-PROF-001-11`, `POL-MBR-PROF-001-12`, `POL-MBR-REJOIN-001-01`, `POL-MBR-REJOIN-001-02`, `POL-MBR-REJOIN-001-03`, `POL-MBR-REJOIN-001-04`, `POL-MBR-REJOIN-001-05`, `POL-MBR-REJOIN-001-06`, `POL-MBR-REJOIN-001-07`, `POL-MBR-REJOIN-001-08`, `POL-MBR-REJOIN-001-09`, `POL-MBR-REJOIN-001-10`, `POL-MBR-REJOIN-004-01`, `POL-MBR-REJOIN-004-02`, `POL-MBR-REJOIN-004-03`, `POL-MBR-REJOIN-004-04`, `POL-MBR-REJOIN-004-05`, `POL-MBR-REJOIN-004-06`, `POL-MBR-REJOIN-004-07`, `POL-MBR-REJOIN-004-08`, `POL-MBR-REJOIN-004-09`, `POL-MBR-REJOIN-004-10`, `POL-MBR-REJOIN-004-11`, `POL-MBR-REJOIN-005-01`, `POL-MBR-REJOIN-005-02`, `POL-MBR-REJOIN-005-03`, `POL-MBR-REJOIN-005-04`, `POL-MBR-REJOIN-005-05`, `POL-MBR-REJOIN-005-06`, `POL-MBR-REJOIN-005-07`, `POL-MBR-REJOIN-005-08`, `POL-MBR-REJOIN-005-09`, `POL-MBR-REJOIN-005-10`, `POL-MBR-ROUTE-001-01`, `POL-MBR-ROUTE-001-02`, `POL-MBR-ROUTE-001-03`, `POL-MBR-ROUTE-001-04`, `POL-MBR-ROUTE-001-05`, `POL-MBR-ROUTE-001-06`, `POL-MBR-ROUTE-001-07`, `POL-MBR-ROUTE-001-08`, `POL-MBR-ROUTE-001-09`, `POL-MBR-ROUTE-001-10`, `POL-MBR-ROUTE-001-11`, `POL-MBR-SESS-001-01`, `POL-MBR-SESS-001-02`, `POL-MBR-SESS-001-03`, `POL-MBR-SESS-001-04`, `POL-MBR-SESS-001-05`, `POL-MBR-SESS-001-06`, `POL-MBR-SESS-001-07`, `POL-MBR-SESS-001-08`, `POL-MBR-SESS-001-09`, `POL-MBR-SESS-001-10`, `POL-MBR-SESS-001-11`, `POL-MBR-SESS-002-01`, `POL-MBR-SESS-002-02`, `POL-MBR-SESS-002-03`, `POL-MBR-SESS-002-04`, `POL-MBR-SESS-002-05`, `POL-MBR-SESS-002-06`, `POL-MBR-SESS-002-07`, `POL-MBR-SESS-002-08`, `POL-MBR-SESS-002-09`, `POL-MBR-SESS-002-10`, `POL-MBR-SESS-002-11`, `POL-MBR-SESS-002-12`, `POL-MBR-STAT-001-01`, `POL-MBR-STAT-001-02`, `POL-MBR-STAT-001-03`, `POL-MBR-STAT-001-04`, `POL-MBR-STAT-001-05`, `POL-MBR-STAT-001-06`, `POL-MBR-STAT-001-07`, `POL-MBR-STAT-001-08`, `POL-MBR-STAT-001-09`, `POL-MBR-STAT-001-10`, `POL-MBR-STAT-001-11`, `POL-MBR-STAT-001-12`, `POL-MBR-TERM-001-01`, `POL-MBR-TERM-001-02`, `POL-MBR-TERM-001-03`, `POL-MBR-TERM-001-04`, `POL-MBR-TERM-001-05`, `POL-MBR-TERM-001-06`, `POL-MBR-TERM-001-07`, `POL-MBR-TERM-001-08`, `POL-MBR-TERM-001-09`, `POL-MBR-TERM-001-10`, `POL-MBR-TERM-001-11`, `POL-MBR-TERM-001-12`, `POL-MBR-TERM-002-01`, `POL-MBR-TERM-002-02`, `POL-MBR-TERM-002-03`, `POL-MBR-TERM-002-04`, `POL-MBR-TERM-002-05`, `POL-MBR-TERM-002-06`, `POL-MBR-TERM-002-07`, `POL-MBR-TERM-002-08`, `POL-MBR-TERM-002-09`, `POL-MBR-TERM-002-10`, `POL-MBR-TERM-003-01`, `POL-MBR-TERM-003-02`, `POL-MBR-TERM-003-03`, `POL-MBR-TERM-003-04`, `POL-MBR-TERM-003-05`, `POL-MBR-TERM-003-06`, `POL-MBR-TERM-003-07`, `POL-MBR-TERM-003-08`, `POL-MBR-TERM-003-09`, `POL-MBR-TERM-003-10`, `POL-MBR-TERM-003-11`, `POL-MBR-TERM-003-12`, `PR-MBR-CS-001-01`, `PR-MBR-CS-001-02`, `PR-MBR-CS-001-03`, `PR-MBR-CS-001-04`, `PR-MBR-CS-001-05`, `US-MBR-CS-001`
