---
유즈케이스_ID: US-MBR-CS-004
액터: 고객
설명: 기존 가입 이력이 있는 고객이 다시 회원으로 전환되기 위해 수행하는 유즈케이스
프로세스_정의_대상: Y
---

# Usecase: US-MBR-CS-004 — 재가입
...
graph LR
    ...
    Start --> PR_MBR_CS_004_01
    PR_MBR_CS_004_02[기존 회원 이력 확인<br/>PR-MBR-CS-004-02]
    PR_MBR_CS_004_01 --> PR_MBR_CS_004_02
    PR_MBR_CS_004_03[재가입 가능 여부 확인<br/>PR-MBR-CS-004-03]
    PR_MBR_CS_004_02 --> PR_MBR_CS_004_03
    PR_MBR_CS_004_04[개인정보 입력<br/>PR-MBR-CS-004-04]
    PR_MBR_CS_004_03 --> PR_MBR_CS_004_04
    PR_MBR_CS_004_05[재가입 처리<br/>PR-MBR-CS-004-05]
    PR_MBR_CS_004_04 --> PR_MBR_CS_004_05
    End((종료))
    PR_MBR_CS_004_05 --> End
```

## Process: PR-MBR-CS-004-01 — 본인인증 {#process-PR-MBR-CS-004-01}

```yaml
프로세스_ID: PR-MBR-CS-004-01
프로세스명: 본인인증
설명: 재가입 대상 고객의 본인 여부를 확인한다.
관련_기능: [FN-MBR-COM-002, FN-MBR-REJOIN-001]
```
...
### Function: FN-MBR-REJOIN-001

```yaml
기능_ID: FN-MBR-REJOIN-001
기능명: 재가입 대상 확인
설명: 본인인증 결과를 기준으로 탈퇴 이력과 재가입 대상 여부를 확인한다.
관련_정책_그룹: [PG-MBR-AUTH-001, PG-MBR-AUTH-002, PG-MBR-AUTH-003, PG-MBR-AUTH-004, PG-MBR-AUTH-005, PG-MBR-AUTH-006, PG-MBR-AUTH-007, PG-MBR-STAT-001, PG-MBR-REJOIN-001]
```

| 항목 | 내용 |
| --- | --- |
| 입력 정보 | CI/DI, 인증세션ID, 탈퇴이력, 회원상태 |
| 세부 기능 구성 | 본인인증 정보 확인 탈퇴 이력 조회 기존 회원 식별 재가입 대상 여부 반환 |
| 출력 정보 | 재가입대상여부, 기존회원ID, 탈퇴일시, 재가입가능일, 다음 업무 |
| 처리 흐름 | (상태) 탈퇴 이력 존재 → (액션) 재가입 대상 여부 조회 → (결과) 재가입 플로우 진입 (상태) 정상 회원 존재 → (액션) 재가입 대상 제외 → (결과) 로그인 안내 (상태) 탈퇴 이력 없음 → (액션) 신규 가입 대상 판정 → (결과) 가입 플로우 안내 |
| 실패/예외 케이스 | 정상 회원 존재: 로그인 안내 이력 조회 실패: 재시도 안내 식별정보 불일치: 본인인증 재수행 |
...
#### Policy Group: PG-MBR-AUTH-003

```yaml
정책_ID: PG-MBR-AUTH-003
정책명: 인증번호 발급 정책
설명: 인증번호 방식의 인증에서 인증번호 생성, 발급, 유효성 기준을 정의하는 정책 그룹이다.
```

| Policy Item ID | 정책 항목명 | 정책 항목 |
| --- | --- | --- |
| `POL-MBR-AUTH-003-01` | 인증번호 자리수 | - 6자리 |
| `POL-MBR-AUTH-003-02` | 인증번호 문자 구성 | - 숫자 |
| `POL-MBR-AUTH-003-03` | 인증번호 유효시간 | - 3분 |
| `POL-MBR-AUTH-003-04` | 인증번호 발급 채널 | - SMS |
| `POL-MBR-AUTH-003-05` | 인증번호 재발급 시 기존 번호 처리 | - 기존 인증번호 즉시 무효화 |
| `POL-MBR-AUTH-003-06` | 인증번호 원문 저장 여부 | - 저장 불가 |
| `POL-MBR-AUTH-003-07` | 인증번호 발송 문구 템플릿 | - [SKT] 본인확인 인증번호는 {인증번호}입니다. 3분 이내 입력해 주세요. |

#### Policy Group: PG-MBR-AUTH-004

```yaml
정책_ID: PG-MBR-AUTH-004
정책명: 인증번호 재요청 정책
설명: 인증번호 재요청 가능 시간, 가능 횟수, 초기화 기준을 정의하는 정책 그룹이다.
```

| Policy Item ID | 정책 항목명 | 정책 항목 |
| --- | --- | --- |
| `POL-MBR-AUTH-004-01` | 인증번호 재요청 가능 시간 | - 발급 후 60초 |
| `POL-MBR-AUTH-004-02` | 인증번호 재요청 가능 횟수 | - 동일 인증 세션 내 최대 5회 |
| `POL-MBR-AUTH-004-03` | 재요청 횟수 초기화 기준 | - 인증 세션 종료 시 초기화 |
| `POL-MBR-AUTH-004-04` | 재요청 제한 시 처리 | - 재요청 버튼 비활성화 |
| `POL-MBR-AUTH-004-05` | 재요청 후 기존 번호 처리 | - 기존 인증번호 즉시 무효화 |
...
## Process: PR-MBR-CS-004-02 — 기존 회원 이력 확인 {#process-PR-MBR-CS-004-02}

```yaml
프로세스_ID: PR-MBR-CS-004-02
프로세스명: 기존 회원 이력 확인
설명: 기존 가입, 탈퇴, 휴면 이력을 조회한다.
관련_기능: [FN-MBR-COM-001, FN-MBR-REJOIN-001, FN-MBR-REJOIN-003]
```
...
### Function: FN-MBR-REJOIN-003

```yaml
기능_ID: FN-MBR-REJOIN-003
기능명: 기존 이력 연계 및 복원 범위 판정
설명: 기존 이력의 연계 가능 범위와 복원 제외 대상을 판정한다.
관련_정책_그룹: [PG-MBR-STAT-001, PG-MBR-REJOIN-001, PG-MBR-REJOIN-002, PG-MBR-REJOIN-003]
```

| 항목 | 내용 |
| --- | --- |
| 입력 정보 | 기존회원ID, CI/DI, 탈퇴이력, 보관 데이터 목록 |
| 세부 기능 구성 | 기존 회원 이력 조회 복원 가능 정보 확인 재사용 가능 정보 확인 신규 생성 대상 분리 |
| 출력 정보 | 연계가능항목, 복원제외항목, 기존이력ID, 신규생성대상 |
| 처리 흐름 | (상태) 법정 보관 이력 존재 → (액션) 연계 가능 항목 판정 → (결과) 재가입 세션에 연결 (상태) 복원 제외 대상 존재 → (액션) 제외 사유 안내 → (결과) 신규 생성 대상 표시 (상태) 연계 가능 정보 없음 → (액션) 신규 가입 기준 적용 → (결과) 신규 프로필 생성 대상 반환 |
| 실패/예외 케이스 | 보관 기간 만료: 신규 생성 안내 이력 불일치: 수동 확인 안내 복원 제한 항목 요청: 제외 안내 |
...
#### Policy Group: PG-MBR-REJOIN-002

```yaml
정책_ID: PG-MBR-REJOIN-002
정책명: 재가입 가능 여부 판정 정책
설명: 탈퇴 이력, 제한 기간, 고객 상태를 기준으로 재가입 가능 여부를 판정하는 정책 그룹이다.
```

| Policy Item ID | 정책 항목명 | 정책 항목 |
| --- | --- | --- |
| `POL-MBR-REJOIN-002-01` | 재가입 가능 상태 | - 탈퇴 확정 |
| `POL-MBR-REJOIN-002-02` | 재가입 불가 상태 | - 정상, 휴면, 가입제한 |
| `POL-MBR-REJOIN-002-03` | 재가입 제한 기간 | - 탈퇴 요청일로부터 7일 |
| `POL-MBR-REJOIN-002-04` | 탈퇴 이력 기준 | - CI |
| `POL-MBR-REJOIN-002-05` | 중복 계정 기준 | - CI, DI, 휴대폰번호 |
| `POL-MBR-REJOIN-002-06` | 재가입 가능 채널 | - 앱, 모바일웹, PC웹 |
| `POL-MBR-REJOIN-002-07` | 재가입 불가 안내 항목 | - 불가 사유, 후속 문의 경로 |
| `POL-MBR-REJOIN-002-08` | 판정 수행 시스템 | - BSS |
| `POL-MBR-REJOIN-002-09` | 판정 실패 처리 | - 업무 중단 및 오류 안내 |
| `POL-MBR-REJOIN-002-10` | 판정 이력 저장 항목 | - CI, 회원ID, 판정 결과, 판정 일시, 처리 채널 |

#### Policy Group: PG-MBR-REJOIN-003

```yaml
정책_ID: PG-MBR-REJOIN-003
정책명: 재가입 제한 및 예외 적용 정책
설명: 재가입 제한 조건과 예외 승인 가능 조건을 정의하는 정책 그룹이다.
```

| Policy Item ID | 정책 항목명 | 정책 항목 |
| --- | --- | --- |
| `POL-MBR-REJOIN-003-01` | 제한 대상 | - 가입제한 회원, 부정이용 이력 회원, 법령·약관 위반 회원 |
| `POL-MBR-REJOIN-003-02` | 제한 기준 식별자 | - CI, DI |
| `POL-MBR-REJOIN-003-03` | 제한 기간 | - 제한 사유별 운영 정책에 따르며 기본 1년 |
| `POL-MBR-REJOIN-003-04` | 예외 승인 조건 | - 운영자 승인 |
| `POL-MBR-REJOIN-003-05` | 예외 승인 권한 | - 관리자 |
| `POL-MBR-REJOIN-003-06` | 예외 승인 증빙 | - 본인확인 결과, 제한 해소 증빙, 운영자 승인 사유 |
| `POL-MBR-REJOIN-003-07` | 예외 승인 후 처리 | - 재가입 가능 |
| `POL-MBR-REJOIN-003-08` | 예외 거절 처리 | - 재가입 불가 안내 |
| `POL-MBR-REJOIN-003-09` | 제한 해제 조건 | - 제한 기간 만료 또는 운영자 예외 승인 완료 |
| `POL-MBR-REJOIN-003-10` | 예외 이력 저장 항목 | - 고객ID, CI, 승인자ID, 승인 일시, 승인 결과 |

## Process: PR-MBR-CS-004-03 — 재가입 가능 여부 확인 {#process-PR-MBR-CS-004-03}

```yaml
프로세스_ID: PR-MBR-CS-004-03
프로세스명: 재가입 가능 여부 확인
설명: 재가입 제한 및 가능 조건을 확인한다.
관련_기능: [FN-MBR-REJOIN-002, FN-MBR-REJOIN-003]
```
...
### Function: FN-MBR-REJOIN-002

```yaml
기능_ID: FN-MBR-REJOIN-002
기능명: 재가입 제한 여부 판정
설명: 탈퇴 이력, 제한 기간, 가입 제한 사유를 기준으로 재가입 가능 여부를 판정한다.
관련_정책_그룹: [PG-MBR-REJOIN-002, PG-MBR-REJOIN-003]
```

| 항목 | 내용 |
| --- | --- |
| 입력 정보 | CI/DI, 탈퇴일시, 재가입 제한 이력, 부정이용 이력 |
| 세부 기능 구성 | 제한 조건 조회 제한 기간 확인 예외 허용 여부 확인 재가입 가능 여부 반환 |
| 출력 정보 | 재가입가능여부, 제한사유, 제한종료일, 대체 안내 |
| 처리 흐름 | (상태) 제한 기간 종료 → (액션) 재가입 가능 판정 → (결과) 재가입 진행 허용 (상태) 제한 기간 미종료 → (액션) 제한 종료일 안내 → (결과) 진행 제한 (상태) 부정이용 제한 대상 → (액션) 가입 제한 사유 반환 → (결과) 가입 제한 안내 |
| 실패/예외 케이스 | 제한 기간 미종료: 재가입 불가 안내 부정이용 제한: 가입 제한 안내 판정 오류: 재시도 안내 |
...
## Process: PR-MBR-CS-004-04 — 개인정보 입력 {#process-PR-MBR-CS-004-04}

```yaml
프로세스_ID: PR-MBR-CS-004-04
프로세스명: 개인정보 입력
설명: 재가입에 필요한 정보를 입력한다.
관련_기능: [FN-MBR-REJOIN-004, FN-MBR-COM-003, FN-MBR-COM-004]
```
...
### Function: FN-MBR-REJOIN-004

```yaml
기능_ID: FN-MBR-REJOIN-004
기능명: 재가입 정보 입력 및 약관 동의
설명: 재가입에 필요한 회원정보를 갱신하고 최신 약관 동의를 받는다.
관련_정책_그룹: [PG-MBR-TERM-001]
```

| 항목 | 내용 |
| --- | --- |
| 입력 정보 | 회원정보, 기존이력ID, 약관동의값, 연락처, 세션ID |
| 세부 기능 구성 | 재가입 필수 정보 입력 기존 정보 재사용 여부 확인 재가입 약관 동의 입력값 검증 |
| 출력 정보 | 입력검증결과, 약관동의이력ID, 기존정보사용여부, 다음 이동 경로 |
| 처리 흐름 | (상태) 재가입 가능 상태 → (액션) 최신 정보 입력과 약관 동의 수집 → (결과) 재가입 처리 가능 상태 생성 (상태) 기존 정보 사용 가능 → (액션) 확인·수정 방식 제공 → (결과) 입력 부담 최소화 (상태) 필수 동의 미완료 → (액션) 미동의 항목 표시 → (결과) 진행 제한 |
| 실패/예외 케이스 | 필수 정보 누락: 입력 요청 약관 미동의: 진행 불가 기존 정보 사용 불가: 신규 입력 요청 |
...
## Process: PR-MBR-CS-004-05 — 재가입 처리 {#process-PR-MBR-CS-004-05}

```yaml
프로세스_ID: PR-MBR-CS-004-05
프로세스명: 재가입 처리
설명: 재가입을 완료하고 회원 상태를 복원 또는 생성한다.
관련_기능: [FN-MBR-REJOIN-005, FN-MBR-REJOIN-006, FN-MBR-COM-006, FN-MBR-COM-007, FN-MBR-COM-005, FN-MBR-COM-008]
```
...
### Function: FN-MBR-REJOIN-005

```yaml
기능_ID: FN-MBR-REJOIN-005
기능명: 재가입 처리
설명: 재가입 대상 고객의 계정을 복원 또는 신규 생성 방식으로 처리한다.
관련_정책_그룹: [PG-MBR-REJOIN-004, PG-MBR-REJOIN-005]
```

| 항목 | 내용 |
| --- | --- |
| 입력 정보 | CI/DI, 기존이력ID, 회원정보, 약관동의이력ID, 인증세션ID |
| 세부 기능 구성 | 계정 복원 또는 신규 생성 식별정보 재연결 회원 상태 정상 전환 재가입 이력 저장 |
| 출력 정보 | 재가입결과, 회원ID, 회원상태코드, 연계이력ID, 로그인세션ID |
| 처리 흐름 | (상태) 기존 계정 복원 가능 → (액션) 계정 상태를 정상으로 전환 → (결과) 재가입 완료 (상태) 신규 생성 필요 → (액션) 신규 회원ID 생성 및 기존 이력 연결 → (결과) 재가입 완료 (상태) 처리 완료 → (액션) 세션 생성 및 알림 발송 → (결과) 서비스 이용 가능 |
| 실패/예외 케이스 | 계정 복원 실패: 신규 생성 또는 재시도 안내 식별정보 중복: 상태 재조회 알림 실패: 화면 안내 유지 |
...
### Function: FN-MBR-REJOIN-006

```yaml
기능_ID: FN-MBR-REJOIN-006
기능명: 재가입 완료 안내
...
| 세부 기능 구성 | 재가입 완료 결과 제공 완료 알림 발송 로그인 세션 생성 후속 이용 안내 |
| 출력 정보 | 완료안내문구, 연계항목, 제외항목, 후속 액션, 알림결과 |
| 처리 흐름 | (상태) 재가입 완료 → (액션) 완료 화면과 알림 발송 → (결과) 정상 이용 안내 (상태) 기존 이력 일부 연계 → (액션) 연계·제외 항목 안내 → (결과) 고객 확인 제공 (상태) 후속 설정 필요 → (액션) 내정보 점검 액션 제공 → (결과) 후속 작업 유도 |
| 실패/예외 케이스 | 알림 실패: 화면 안내 유지 연계항목 조회 실패: 내정보에서 확인 안내 세션 오류: 로그인 재시도 안내 |
...
- 정의된 ID: `FN-MBR-COM-001`, `FN-MBR-COM-002`, `FN-MBR-COM-003`, `FN-MBR-COM-004`, `FN-MBR-COM-005`, `FN-MBR-COM-006`, `FN-MBR-COM-007`, `FN-MBR-COM-008`, `FN-MBR-REJOIN-001`, `FN-MBR-REJOIN-002`, `FN-MBR-REJOIN-003`, `FN-MBR-REJOIN-004`, `FN-MBR-REJOIN-005`, `FN-MBR-REJOIN-006`, `PG-MBR-ACCT-001`, `PG-MBR-AUTH-001`, `PG-MBR-AUTH-002`, `PG-MBR-AUTH-003`, `PG-MBR-AUTH-004`, `PG-MBR-AUTH-005`, `PG-MBR-AUTH-006`, `PG-MBR-AUTH-007`, `PG-MBR-DORM-001`, `PG-MBR-DORM-002`, `PG-MBR-DORM-003`, `PG-MBR-DORM-004`, `PG-MBR-JOIN-001`, `PG-MBR-LEAVE-008`, `PG-MBR-LEAVE-009`, `PG-MBR-LEAVE-010`, `PG-MBR-LEAVE-011`, `PG-MBR-LEAVE-012`, `PG-MBR-LOGIN-001`, `PG-MBR-PROF-001`, `PG-MBR-REJOIN-001`, `PG-MBR-REJOIN-002`, `PG-MBR-REJOIN-003`, `PG-MBR-REJOIN-004`, `PG-MBR-REJOIN-005`, `PG-MBR-ROUTE-001`, `PG-MBR-SESS-001`, `PG-MBR-SESS-002`, `PG-MBR-STAT-001`, `PG-MBR-TERM-001`, `PG-MBR-TERM-002`, `PG-MBR-TERM-003`, `POL-MBR-ACCT-001-01`, `POL-MBR-ACCT-001-02`, `POL-MBR-ACCT-001-03`, `POL-MBR-ACCT-001-04`, `POL-MBR-ACCT-001-05`, `POL-MBR-ACCT-001-06`, `POL-MBR-ACCT-001-07`, `POL-MBR-ACCT-001-08`, `POL-MBR-ACCT-001-09`, `POL-MBR-ACCT-001-10`, `POL-MBR-ACCT-001-11`, `POL-MBR-ACCT-001-12`, `POL-MBR-AUTH-001-01`, `POL-MBR-AUTH-001-02`, `POL-MBR-AUTH-001-03`, `POL-MBR-AUTH-001-04`, `POL-MBR-AUTH-001-05`, `POL-MBR-AUTH-001-06`, `POL-MBR-AUTH-001-07`, `POL-MBR-AUTH-001-08`, `POL-MBR-AUTH-002-01`, `POL-MBR-AUTH-002-02`, `POL-MBR-AUTH-002-03`, `POL-MBR-AUTH-002-04`, `POL-MBR-AUTH-002-05`, `POL-MBR-AUTH-002-06`, `POL-MBR-AUTH-002-07`, `POL-MBR-AUTH-002-08`, `POL-MBR-AUTH-002-09`, `POL-MBR-AUTH-003-01`, `POL-MBR-AUTH-003-02`, `POL-MBR-AUTH-003-03`, `POL-MBR-AUTH-003-04`, `POL-MBR-AUTH-003-05`, `POL-MBR-AUTH-003-06`, `POL-MBR-AUTH-003-07`, `POL-MBR-AUTH-004-01`, `POL-MBR-AUTH-004-02`, `POL-MBR-AUTH-004-03`, `POL-MBR-AUTH-004-04`, `POL-MBR-AUTH-004-05`, `POL-MBR-AUTH-005-01`, `POL-MBR-AUTH-005-02`, `POL-MBR-AUTH-005-03`, `POL-MBR-AUTH-005-04`, `POL-MBR-AUTH-005-05`, `POL-MBR-AUTH-005-06`, `POL-MBR-AUTH-005-07`, `POL-MBR-AUTH-006-01`, `POL-MBR-AUTH-006-02`, `POL-MBR-AUTH-006-03`, `POL-MBR-AUTH-006-04`, `POL-MBR-AUTH-006-05`, `POL-MBR-AUTH-006-06`, `POL-MBR-AUTH-006-07`, `POL-MBR-AUTH-007-01`, `POL-MBR-AUTH-007-02`, `POL-MBR-AUTH-007-03`, `POL-MBR-AUTH-007-04`, `POL-MBR-AUTH-007-05`, `POL-MBR-AUTH-007-06`, `POL-MBR-AUTH-007-07`, `POL-MBR-DORM-001-01`, `POL-MBR-DORM-001-02`, `POL-MBR-DORM-001-03`, `POL-MBR-DORM-001-04`, `POL-MBR-DORM-001-05`, `POL-MBR-DORM-001-06`, `POL-MBR-DORM-001-07`, `POL-MBR-DORM-001-08`, `POL-MBR-DORM-001-09`, `POL-MBR-DORM-001-10`, `POL-MBR-DORM-001-11`, `POL-MBR-DORM-001-12`, `POL-MBR-DORM-002-01`, `POL-MBR-DORM-002-02`, `POL-MBR-DORM-002-03`, `POL-MBR-DORM-002-04`, `POL-MBR-DORM-002-05`, `POL-MBR-DORM-002-06`, `POL-MBR-DORM-002-07`, `POL-MBR-DORM-002-08`, `POL-MBR-DORM-002-09`, `POL-MBR-DORM-002-10`, `POL-MBR-DORM-002-11`, `POL-MBR-DORM-002-12`, `POL-MBR-DORM-003-01`, `POL-MBR-DORM-003-02`, `POL-MBR-DORM-003-03`, `POL-MBR-DORM-003-04`, `POL-MBR-DORM-003-05`, `POL-MBR-DORM-003-06`, `POL-MBR-DORM-003-07`, `POL-MBR-DORM-003-08`, `POL-MBR-DORM-003-09`, `POL-MBR-DORM-003-10`, `POL-MBR-DORM-003-11`, `POL-MBR-DORM-003-12`, `POL-MBR-DORM-004-01`, `POL-MBR-DORM-004-02`, `POL-MBR-DORM-004-03`, `POL-MBR-DORM-004-04`, `POL-MBR-DORM-004-05`, `POL-MBR-DORM-004-06`, `POL-MBR-DORM-004-07`, `POL-MBR-DORM-004-08`, `POL-MBR-DORM-004-09`, `POL-MBR-DORM-004-10`, `POL-MBR-DORM-004-11`, `POL-MBR-DORM-004-12`, `POL-MBR-JOIN-001-01`, `POL-MBR-JOIN-001-02`, `POL-MBR-JOIN-001-03`, `POL-MBR-JOIN-001-04`, `POL-MBR-JOIN-001-05`, `POL-MBR-JOIN-001-06`, `POL-MBR-JOIN-001-07`, `POL-MBR-JOIN-001-08`, `POL-MBR-JOIN-001-09`, `POL-MBR-JOIN-001-10`, `POL-MBR-JOIN-001-11`, `POL-MBR-JOIN-001-12`, `POL-MBR-LEAVE-008-01`, `POL-MBR-LEAVE-008-02`, `POL-MBR-LEAVE-008-03`, `POL-MBR-LEAVE-008-04`, `POL-MBR-LEAVE-008-05`, `POL-MBR-LEAVE-008-06`, `POL-MBR-LEAVE-008-07`, `POL-MBR-LEAVE-008-08`, `POL-MBR-LEAVE-008-09`, `POL-MBR-LEAVE-008-10`, `POL-MBR-LEAVE-008-11`, `POL-MBR-LEAVE-008-12`, `POL-MBR-LEAVE-009-01`, `POL-MBR-LEAVE-009-02`, `POL-MBR-LEAVE-009-03`, `POL-MBR-LEAVE-009-04`, `POL-MBR-LEAVE-009-05`, `POL-MBR-LEAVE-009-06`, `POL-MBR-LEAVE-009-07`, `POL-MBR-LEAVE-009-08`, `POL-MBR-LEAVE-009-09`, `POL-MBR-LEAVE-009-10`, `POL-MBR-LEAVE-009-11`, `POL-MBR-LEAVE-010-01`, `POL-MBR-LEAVE-010-02`, `POL-MBR-LEAVE-010-03`, `POL-MBR-LEAVE-010-04`, `POL-MBR-LEAVE-010-05`, `POL-MBR-LEAVE-010-06`, `POL-MBR-LEAVE-010-07`, `POL-MBR-LEAVE-010-08`, `POL-MBR-LEAVE-010-09`, `POL-MBR-LEAVE-010-10`, `POL-MBR-LEAVE-010-11`, `POL-MBR-LEAVE-010-12`, `POL-MBR-LEAVE-011-01`, `POL-MBR-LEAVE-011-02`, `POL-MBR-LEAVE-011-03`, `POL-MBR-LEAVE-011-04`, `POL-MBR-LEAVE-011-05`, `POL-MBR-LEAVE-011-06`, `POL-MBR-LEAVE-011-07`, `POL-MBR-LEAVE-011-08`, `POL-MBR-LEAVE-011-09`, `POL-MBR-LEAVE-011-10`, `POL-MBR-LEAVE-011-11`, `POL-MBR-LEAVE-011-12`, `POL-MBR-LEAVE-012-01`, `POL-MBR-LEAVE-012-02`, `POL-MBR-LEAVE-012-03`, `POL-MBR-LEAVE-012-04`, `POL-MBR-LEAVE-012-05`, `POL-MBR-LEAVE-012-06`, `POL-MBR-LEAVE-012-07`, `POL-MBR-LEAVE-012-08`, `POL-MBR-LEAVE-012-09`, `POL-MBR-LEAVE-012-10`, `POL-MBR-LEAVE-012-11`, `POL-MBR-LEAVE-012-12`, `POL-MBR-LOGIN-001-01`, `POL-MBR-LOGIN-001-02`, `POL-MBR-LOGIN-001-03`, `POL-MBR-LOGIN-001-04`, `POL-MBR-LOGIN-001-05`, `POL-MBR-LOGIN-001-06`, `POL-MBR-LOGIN-001-07`, `POL-MBR-LOGIN-001-08`, `POL-MBR-LOGIN-001-09`, `POL-MBR-LOGIN-001-10`, `POL-MBR-LOGIN-001-11`, `POL-MBR-LOGIN-001-12`, `POL-MBR-PROF-001-01`, `POL-MBR-PROF-001-02`, `POL-MBR-PROF-001-03`, `POL-MBR-PROF-001-04`, `POL-MBR-PROF-001-05`, `POL-MBR-PROF-001-06`, `POL-MBR-PROF-001-07`, `POL-MBR-PROF-001-08`, `POL-MBR-PROF-001-09`, `POL-MBR-PROF-001-10`, `POL-MBR-PROF-001-11`, `POL-MBR-PROF-001-12`, `POL-MBR-REJOIN-001-01`, `POL-MBR-REJOIN-001-02`, `POL-MBR-REJOIN-001-03`, `POL-MBR-REJOIN-001-04`, `POL-MBR-REJOIN-001-05`, `POL-MBR-REJOIN-001-06`, `POL-MBR-REJOIN-001-07`, `POL-MBR-REJOIN-001-08`, `POL-MBR-REJOIN-001-09`, `POL-MBR-REJOIN-001-10`, `POL-MBR-REJOIN-002-01`, `POL-MBR-REJOIN-002-02`, `POL-MBR-REJOIN-002-03`, `POL-MBR-REJOIN-002-04`, `POL-MBR-REJOIN-002-05`, `POL-MBR-REJOIN-002-06`, `POL-MBR-REJOIN-002-07`, `POL-MBR-REJOIN-002-08`, `POL-MBR-REJOIN-002-09`, `POL-MBR-REJOIN-002-10`, `POL-MBR-REJOIN-003-01`, `POL-MBR-REJOIN-003-02`, `POL-MBR-REJOIN-003-03`, `POL-MBR-REJOIN-003-04`, `POL-MBR-REJOIN-003-05`, `POL-MBR-REJOIN-003-06`, `POL-MBR-REJOIN-003-07`, `POL-MBR-REJOIN-003-08`, `POL-MBR-REJOIN-003-09`, `POL-MBR-REJOIN-003-10`, `POL-MBR-REJOIN-004-01`, `POL-MBR-REJOIN-004-02`, `POL-MBR-REJOIN-004-03`, `POL-MBR-REJOIN-004-04`, `POL-MBR-REJOIN-004-05`, `POL-MBR-REJOIN-004-06`, `POL-MBR-REJOIN-004-07`, `POL-MBR-REJOIN-004-08`, `POL-MBR-REJOIN-004-09`, `POL-MBR-REJOIN-004-10`, `POL-MBR-REJOIN-004-11`, `POL-MBR-REJOIN-005-01`, `POL-MBR-REJOIN-005-02`, `POL-MBR-REJOIN-005-03`, `POL-MBR-REJOIN-005-04`, `POL-MBR-REJOIN-005-05`, `POL-MBR-REJOIN-005-06`, `POL-MBR-REJOIN-005-07`, `POL-MBR-REJOIN-005-08`, `POL-MBR-REJOIN-005-09`, `POL-MBR-REJOIN-005-10`, `POL-MBR-ROUTE-001-01`, `POL-MBR-ROUTE-001-02`, `POL-MBR-ROUTE-001-03`, `POL-MBR-ROUTE-001-04`, `POL-MBR-ROUTE-001-05`, `POL-MBR-ROUTE-001-06`, `POL-MBR-ROUTE-001-07`, `POL-MBR-ROUTE-001-08`, `POL-MBR-ROUTE-001-09`, `POL-MBR-ROUTE-001-10`, `POL-MBR-ROUTE-001-11`, `POL-MBR-SESS-001-01`, `POL-MBR-SESS-001-02`, `POL-MBR-SESS-001-03`, `POL-MBR-SESS-001-04`, `POL-MBR-SESS-001-05`, `POL-MBR-SESS-001-06`, `POL-MBR-SESS-001-07`, `POL-MBR-SESS-001-08`, `POL-MBR-SESS-001-09`, `POL-MBR-SESS-001-10`, `POL-MBR-SESS-001-11`, `POL-MBR-SESS-002-01`, `POL-MBR-SESS-002-02`, `POL-MBR-SESS-002-03`, `POL-MBR-SESS-002-04`, `POL-MBR-SESS-002-05`, `POL-MBR-SESS-002-06`, `POL-MBR-SESS-002-07`, `POL-MBR-SESS-002-08`, `POL-MBR-SESS-002-09`, `POL-MBR-SESS-002-10`, `POL-MBR-SESS-002-11`, `POL-MBR-SESS-002-12`, `POL-MBR-STAT-001-01`, `POL-MBR-STAT-001-02`, `POL-MBR-STAT-001-03`, `POL-MBR-STAT-001-04`, `POL-MBR-STAT-001-05`, `POL-MBR-STAT-001-06`, `POL-MBR-STAT-001-07`, `POL-MBR-STAT-001-08`, `POL-MBR-STAT-001-09`, `POL-MBR-STAT-001-10`, `POL-MBR-STAT-001-11`, `POL-MBR-STAT-001-12`, `POL-MBR-TERM-001-01`, `POL-MBR-TERM-001-02`, `POL-MBR-TERM-001-03`, `POL-MBR-TERM-001-04`, `POL-MBR-TERM-001-05`, `POL-MBR-TERM-001-06`, `POL-MBR-TERM-001-07`, `POL-MBR-TERM-001-08`, `POL-MBR-TERM-001-09`, `POL-MBR-TERM-001-10`, `POL-MBR-TERM-001-11`, `POL-MBR-TERM-001-12`, `POL-MBR-TERM-002-01`, `POL-MBR-TERM-002-02`, `POL-MBR-TERM-002-03`, `POL-MBR-TERM-002-04`, `POL-MBR-TERM-002-05`, `POL-MBR-TERM-002-06`, `POL-MBR-TERM-002-07`, `POL-MBR-TERM-002-08`, `POL-MBR-TERM-002-09`, `POL-MBR-TERM-002-10`, `POL-MBR-TERM-003-01`, `POL-MBR-TERM-003-02`, `POL-MBR-TERM-003-03`, `POL-MBR-TERM-003-04`, `POL-MBR-TERM-003-05`, `POL-MBR-TERM-003-06`, `POL-MBR-TERM-003-07`, `POL-MBR-TERM-003-08`, `POL-MBR-TERM-003-09`, `POL-MBR-TERM-003-10`, `POL-MBR-TERM-003-11`, `POL-MBR-TERM-003-12`, `PR-MBR-CS-004-01`, `PR-MBR-CS-004-02`, `PR-MBR-CS-004-03`, `PR-MBR-CS-004-04`, `PR-MBR-CS-004-05`, `US-MBR-CS-004`
