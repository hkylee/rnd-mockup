...
3. **아이콘 SVG path 없음**: `docs/components/atom/icon.json` 은 크기 정보만, 실제 경로 없음 → 손그림 SVG로 대체 → 디자인 불일치
...
```
1. docs/components/{tier}/{name}.json 에서 figmaNodeId 확인
...
1. **컴포넌트마다 figmaNodeId를 먼저 확인** → docs/components에 이미 다 있음
